...
class DocumentService:
    ...
    def get_service_documents(self, service_id: int) -> List[str]:
        ...
        document_mapping = {
            ...
            7: ['Квитанция.xlsx', 'Согласие.docx', 'Договор подряда.docx', 
                'Декларация.xlsx', 'Акт к договору подряда.docx'],  # Технический план
            8: ['Квитанция.xlsx', 'Согласие.docx', 'Договор подряда.docx'],  # Уточнение
        }
        return document_mapping.get(service_id, [])
    
    def copy_documents_to_client_folder(self, client: Client) -> List[str]:
        ...
        if self.docs_folder is None:
            raise ValueError("Папка шаблонов не инициализирована")
    ...
    def fill_client_documents(self, client_id: int, selected_docs: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if selected_docs:
            doc_mapping = {
                'approval': ('Согласие.docx', 'Согласие.docx'),  # Заменяем исходный файл
                ...
                'declaration': ('Декларация.xlsx', 'Декларация.xlsx'),
            ...
            if client.doc_fill_info:
                ...
                if client.doc_fill_info.declaration:
                    docs_to_fill['Декларация.xlsx'] = 'Декларация.xlsx'
