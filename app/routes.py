...
def fill_client_documents(client_id):
    """Заполнение документов клиента с автоматическим копированием"""
    client = Client.query.get_or_404(client_id)
    ...
    try:
        ...
        # Заполняем документы с автоматическим копированием
        result = document_service.fill_client_documents(
            client_id, 
            selected_docs, 
            auto_copy=True  # Включаем автокопирование
        )
        
        # Формируем сообщение для пользователя
        message_parts = []
        if result['copied_documents']:
            message_parts.append(f"Скопированы: {', '.join(result['copied_documents'])}")
        if result['filled_documents']:
            message_parts.append(f"Заполнены: {', '.join(result['filled_documents'])}")
        ...
        return jsonify({
            ...
            'message': '; '.join(message_parts) if message_parts else 'Операция завершена'
        })
...
def open_client_folder(client_id):
    ...
    try:
        ...
        if system == "Windows":
            # Для Windows используем subprocess для лучшего контроля
            # /select открывает папку и выделяет её в проводнике
    ...
    # Возвращаем JSON ответ вместо редиректа для AJAX запросов
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return jsonify({'success': True, 'message': 'Папка открыта'})
    
    # Для обычных запросов возвращаемся назад
...
def create_client_folder(client, city, address, move_from=None):
    ...
    if move_from and os.path.exists(move_from) and move_from != folder_path:
        ...
        print(f"✅ Папка перемещена: {move_from} -> {folder_path}")
    
    # Получаем абсолютный путь
    abs_folder_path = os.path.abspath(folder_path)
    
    # Если папка была только что создана, копируем все документы для услуги
    if folder_created:
        print(f"📄 Копируем документы для новой папки клиента...")
        
        try:
            # Используем новый метод, который принимает путь напрямую
            ...
            if copy_result['success'] and copy_result['copied_files']:
                print(f"✅ Скопированы документы: {', '.join(copy_result['copied_files'])}")
            elif copy_result['errors']:
                print(f"⚠️ Ошибки при копировании документов: {', '.join(copy_result['errors'])}")
            else:
                print(f"ℹ️ Документы не копировались (возможно, шаблоны не найдены)")
...
def settings_post():
    ...
    try:
        # Получаем новые настройки из формы (убрали upload_folder)
        ...
        # Проверяем и создаем папки если нужно (убрали upload_folder)
        folders_to_check = [
        ...
        # Обновляем настройки (убрали UPLOAD_FOLDER)
        settings_to_update = {
